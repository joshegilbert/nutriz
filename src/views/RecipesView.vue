      ...
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center">
          ...
          <div class="text-subtitle-2">
            ...
            <span class="ml-2">
              Cal:
              <b>{{ recipeTotalMacros.calories.toFixed(0) }}</b>
            </span>
            <span class="ml-3">
              Prot:
              <b>{{ recipeTotalMacros.protein.toFixed(0) }}g</b>
            </span>
            <span class="ml-3">
              Carb:
              <b>{{ recipeTotalMacros.carbs.toFixed(0) }}g</b>
            </span>
            <span class="ml-3">
              Fat:
              <b>{{ recipeTotalMacros.fat.toFixed(0) }}g</b>
            </span>
          </div>
        </v-card-title>
        <v-card-text style="max-height: 70vh; overflow-y: auto">
          <v-form ref="form">
            ...
            <h3 class="text-h6 mb-2" v-if="editedItem">Ingredients</h3>
...
const { recipes, foods, meals, isLoadingRecipes, lastError } =
...
const recipeTotalMacros = computed(() => {
  if (!editedItem.value?.ingredients) {
    return { calories: 0, protein: 0, carbs: 0, fat: 0 };
  }

  return editedItem.value.ingredients.reduce(
    (totals, ingredient) => {
      const food = foods.value.find((f) => f.id === ingredient.foodItem);
      if (!food) return totals;
      const quantity = Number(ingredient.quantity) || 0;
      totals.calories += (food.macrosPerServing.calories || 0) * quantity;
      totals.protein += (food.macrosPerServing.protein || 0) * quantity;
      totals.carbs += (food.macrosPerServing.carbs || 0) * quantity;
      totals.fat += (food.macrosPerServing.fat || 0) * quantity;
      return totals;
    },
...
function removeIngredient(index) {
  if (!editedItem.value?.ingredients) return;
...
function updateMealComponent(componentIndex, mealId) {
  ...
  if (mealTemplate) {
    component.components = JSON.parse(
      JSON.stringify(mealTemplate.components)
    ).map((c) => ({
      ...c,
